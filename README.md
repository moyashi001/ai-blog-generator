...
1. 商品名（必須）・型番（任意）・Amazon商品URL（任意）・楽天/Yahoo!商品URL（任意）を入力
...
1. 商品名入力フォーム
2. 型番入力（任意・楽天/Yahoo!検索で商品名より優先）
3. Amazon商品URL入力（任意・手動）
4. 楽天市場・Yahoo!ショッピングの商品リンクを商品名（または型番）から自動検索、手動URL入力での上書きも可能
5. もしもアフィリエイト経由のアフィリエイトリンク自動生成
6. Claude Pro貼り付け用のブログ記事生成プロンプトを自動生成（WordPress SWELLにそのまま貼り付けられるHTML出力を指示）
7. 生成されたプロンプトのコピーUI
