...
7. 狙いたいSEOキーワード入力（任意・カンマ区切り。1つ目がメイン、残りがサブキーワード扱い。未入力ならClaude側が自動選定）
8. Claude Pro貼り付け用のブログ記事生成プロンプトを自動生成。以下のSEO・集客対策を指示に含める
   - タイトル案3つ（数字型／悩み共感型／比較型の異なる切り口）
   - メタディスクリプション（120〜160字）
   - SEOキーワード（メイン／サブに分類）
   - よくある質問セクション（Q&A形式、Googleの強調スニペット狙い）
   - まとめ直前の内部リンク導線の一文
   - 画像altへの商品名＋型番を含む説明的なテキスト設定
   - 一次体験らしい具体的な数字（使用期間など）を盛り込む指示（E-E-A-T対策）
   - WordPress SWELLにそのまま貼り付けられるHTML出力、ブログ「もや暮らし」の文体踏襲、アフィリエイトリンクの商品画像＋ボタンのカード形式出力
9. 生成されたプロンプトのコピーUI
10. PWA対応（ホーム画面追加・オフラインキャッシュ）
