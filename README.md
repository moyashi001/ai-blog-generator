...
3. Amazon（常に手動入力）・楽天・Yahoo!それぞれのURLを `MOSHIMO_LINK_TEMPLATE_*` に差し込み、もしもアフィリエイト経由のリンクを生成。楽天・Yahoo!は検索結果の商品画像URLも取得する（Amazonは画像URLも手動入力）
4. 商品名（または型番）・アフィリエイトリンク（画像URL付き）・画像情報・文体ガイド（もや暮らし https://moyagurashi.com/ の文体を踏襲する指示）・アフィリエイトリンクをカード形式（画像+ボタン）のHTMLで出力する指示を埋め込んだプロンプト文字列を返す
...
2. Amazon商品URL・商品画像URL入力（任意・手動）
3. 楽天市場・Yahoo!ショッピングの商品リンク・商品画像URLを商品名（または型番）から自動検索、手動URL入力での上書きも可能
4. もしもアフィリエイト経由のアフィリエイトリンク自動生成
5. 記事に載せる画像のURL・内容入力（任意）→ プロンプトに`<img>`タグ挿入の指示として反映
6. Claude Pro貼り付け用のブログ記事生成プロンプトを自動生成（WordPress SWELLにそのまま貼り付けられるHTML出力、ブログ「もや暮らし」の文体を踏襲する指示、アフィリエイトリンクを商品画像＋ボタンのカード形式で出力する指示）
7. 生成されたプロンプトのコピーUI
