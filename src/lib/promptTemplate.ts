...
【SEO・タイトルの指示】
内部的な作業順序として、まずキーワード選定を行ってから、そのキーワードに基づいてタイトル案とメタディスクリプションを作成してください（出力する順番は後述の【出力形式】の通りで構いません）。
- {{KEYWORD_INSTRUCTION}}
- 選定したメインキーワードは、少なくとも1つのh2見出しにそのままの形（フレーズを分割しない）で含めてください。他の見出しにはサブキーワードを自然に含めてください。
- 上記で選定したキーワードを踏まえて、記事タイトル案を3つ提案してください。32文字以内を目安に、数字を使ったもの／悩み共感型／比較型など異なる切り口で1つずつ作成してください（【】や数字を使うとクリックされやすくなります）。
- 上記で選定したキーワードを踏まえて、検索結果に表示されるメタディスクリプションを120〜160字で1つ作成してください（記事の要点＋読むメリットが伝わる文にする）。
...
- 記事本文（HTML部分）の合計文字数は2,500〜3,500字を目安にしてください。
...
function formatKeywordInstruction(targetKeywords?: string): string {
  ...
  if (keywords.length === 0) {
    return "メインキーワード・サブキーワードの指定はありません。Web検索が使える場合は、商品名・型番・カテゴリの実際の検索需要や競合状況を調べたうえで、単体では競合が激しすぎる一般語（例:「モバイルバッテリー」）ではなく、特徴語や型番と掛け合わせた複合キーワード（例:「モバイルバッテリー 薄型」）をメインキーワードとして1つ、関連するサブキーワードを3〜4つ選定してください。Web検索が使えない場合は、同様の考え方で自分の知識から選定してください。選定したキーワードはタイトル・h2見出し・導入文・まとめに自然な形で含め（詰め込みすぎは避ける）、【SEOキーワード】にメイン／サブの区別が分かる形で明記してください。";
  }
